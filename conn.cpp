...
RET_CODE conn::read_clt() //从套接字中的信息传到缓冲区
{
    int bytes_read = 0;
    while( true )
    {
    ...
    return ( ( m_clt_read_idx - m_clt_write_idx ) > 0 ) ? OK : NOTHING;  //表明缓冲区有数据可以发送了
}

RET_CODE conn::read_srv()   //从套接字中的信息传到缓冲区
{
    int bytes_read = 0;
    while( true )
    {
        ...
        if ( bytes_read == -1 )
        {
            if( errno == EAGAIN || errno == EWOULDBLOCK )  //意味着缓冲区暂时没有更多数据读取，数据尚未到达（对于网络通信而言），操作被设为非阻塞模式，并且当前没有可用的数据
            {
                break;  
            }
            return IOERR;
...
RET_CODE conn::write_srv()  //把缓冲区中的信息传到套接字
{
    int bytes_write = 0;
    while( true )
    {
...
RET_CODE conn::write_clt()   //把缓冲区中的信息传到套接字
{
    int bytes_write = 0;
    while( true )
    {
